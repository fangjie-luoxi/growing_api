...
// GetGrTasks 获取符合条件的GrTask
func GetGrTasks(c *gin.Context) {
	m := models.GrTask{}
...
// GetGrTaskById 获取单个GrTask
func GetGrTaskById(c *gin.Context) {
	idStr := c.Param("id")
...
// CreateGrTask 创建GrTask
func CreateGrTask(c *gin.Context) {
	m := models.GrTask{}
...
// UpdateGrTask 更新GrTask
func UpdateGrTask(c *gin.Context) {
	idStr := c.Param("id")
	id, err := strconv.Atoi(idStr)
	if err != nil {
...
// UpdateFullGrTask 更新关联字段
func UpdateFullGrTask(c *gin.Context) {
	idStr := c.Param("id")
	id, err := strconv.Atoi(idStr)
	if err != nil {
		global.Resp.Error(c, 400, err, "id不能为空")
...
// UpdateGrTasks 更新多个
func UpdateGrTasks(c *gin.Context) {
	m := models.GrTask{}
	var v []*models.GrTask
...
// UpdateGrTaskByQuery 通过查询条件更新多个
func UpdateGrTaskByQuery(c *gin.Context) {
	var v map[string]interface{}
	err := c.ShouldBindBodyWith(&v, binding.JSON)
	if err != nil {
		global.Resp.Error(c, 400, err, "")
